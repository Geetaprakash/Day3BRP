...
class DayOfweek {
	...
	static int mm;
	static int yyyy;
	DayOfweek(int dd, int mm, int yyyy)
	{
		this.dd = dd;
		this.mm = mm;
		this.yyyy = yyyy;
	...
	static void calculate()
	{
		...
		int leapy = checkLeap(yyyy);

		/*Declaring and initialising the given informations and arrays*/
	...
	public static void main(String[] args)
	{
		...
		/*Take i/p in str format&then convert to int values*/
		String date = sc.next();
		String[] values = date.split("/");
		new DayOfweek(Integer.parseInt(values[0]),Integer.parseInt(values[1]),Integer.parseInt(values[2]));
		calculate();
	}
}
